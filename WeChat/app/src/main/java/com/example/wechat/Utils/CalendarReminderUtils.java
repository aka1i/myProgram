...
public class CalendarReminderUtils {
    ...
    private static long addCalendarAccount(Context context) {
        ...
        value.put(CalendarContract.Calendars.NAME, CALENDARS_NAME); //日历名称
        value.put(CalendarContract.Calendars.ACCOUNT_NAME, CALENDARS_ACCOUNT_NAME);//账号名
        value.put(CalendarContract.Calendars.ACCOUNT_TYPE, CALENDARS_ACCOUNT_TYPE);
        value.put(CalendarContract.Calendars.CALENDAR_DISPLAY_NAME, CALENDARS_DISPLAY_NAME);//显示给用户的名字
        value.put(CalendarContract.Calendars.VISIBLE, 1);//为0表示不应显示与此日历关联的事件。值为1表示应显示与此日历关联的事件
        value.put(CalendarContract.Calendars.CALENDAR_COLOR, Color.BLUE);
        value.put(CalendarContract.Calendars.CALENDAR_ACCESS_LEVEL, CalendarContract.Calendars.CAL_ACCESS_OWNER);
        value.put(CalendarContract.Calendars.SYNC_EVENTS, 1);//值为0表示不同步此日历或​​将其事件存储在设备上。值为1表示此日历的同步事件，并将其事件存储在设备上。
        value.put(CalendarContract.Calendars.CALENDAR_TIME_ZONE, timeZone.getID());//时区
    ...
    public static void updateCalendarEvent(Context context,  String oldtitle,String title,String detail,long oldDeadline,long deadline){
        ...
        Log.d(TAG, "old: " + new Date(oldDeadline).toString() + ", \nnew: " + new Date(deadline).toString());
        ...
        try {
            ...
            Log.d(TAG,"curSize: " + eventCursor.getCount());
            if (eventCursor.getCount() > 0) {
                //遍历所有事件，找到title跟需要查询的titl且开始时间一样的项
     ...
     * 删除日历事件
     */
    public static void deleteCalendarEvent(Context context, String title, long deadline) {
        if (context == null) {
            return;
        }
        Cursor eventCursor = context.getContentResolver().query(CALENDER_EVENT_URL, null, null, null, null);
        try {
            ...
            if (eventCursor.getCount() > 0) {
                ...
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    String eventTitle = eventCursor.getString(eventCursor.getColumnIndex("title"));
                    long eventStart = eventCursor.getLong(eventCursor.getColumnIndex(CalendarContract.Events.DTSTART));
                    if (title.equals(eventTitle) && deadline == eventStart) {
                        int id = eventCursor.getInt(eventCursor.getColumnIndex(CalendarContract.Calendars._ID));//取得id
